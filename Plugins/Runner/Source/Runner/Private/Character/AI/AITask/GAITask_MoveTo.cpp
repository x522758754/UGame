...
#include "Character/AI/AITask/GAITask_MoveTo.h"
...
UGAITask_MoveTo::UGAITask_MoveTo(const FObjectInitializer& ObjectInitializer)
...
UGAITask_MoveTo* UGAITask_MoveTo::AIMoveTo(AAIController* Controller, FVector InGoalLocation, AActor* InGoalActor,
...
{
	UGAITask_MoveTo* MyTask = Controller ? UAITask::NewAITask<UGAITask_MoveTo>(*Controller, EAITaskPriority::High) : nullptr;
	if (MyTask)
	{
...
void UGAITask_MoveTo::SetUp(AAIController* Controller, const FAIMoveRequest& InMoveRequest)
{
	OwnerController = Controller;
	MoveRequest = InMoveRequest;
}

void UGAITask_MoveTo::SetContinuousGoalTracking(bool bEnable)
{
	bUseContinuousTracking = bEnable;
}

void UGAITask_MoveTo::FinishMoveTask(EPathFollowingResult::Type InResult)
{
...
void UGAITask_MoveTo::ConditionalPerformMove()
{
	if (MoveRequest.IsUsingPathfinding() && OwnerController && OwnerController->ShouldPostponePathUpdates())
	{
		...
		OwnerController->GetWorldTimerManager().SetTimer(MoveRetryTimerHandle, this, &UGAITask_MoveTo::ConditionalPerformMove, 0.2f, false);
	}
	else
	{
...
void UGAITask_MoveTo::PerformMove()
{
	UPathFollowingComponent* PFComp = OwnerController ? OwnerController->GetPathFollowingComponent() : nullptr;
	if (PFComp == nullptr)
	{
	...
	case EPathFollowingRequestResult::RequestSuccessful:
		MoveRequestID = ResultData.MoveId;
		PathFinishDelegateHandle = PFComp->OnRequestFinished.AddUObject(this, &UGAITask_MoveTo::OnRequestFinished);
...
void UGAITask_MoveTo::SetObservedPath(FNavPathSharedPtr InPath)
{
	...
	if (Path.IsValid())
	{
		...
		PathUpdateDelegateHandle = Path->AddObserver(FNavigationPath::FPathObserverDelegate::FDelegate::CreateUObject(this, &UGAITask_MoveTo::OnPathEvent));
	}
}

void UGAITask_MoveTo::ResetObservers()
{
	if (Path.IsValid())
	{
...
void UGAITask_MoveTo::ResetTimers()
{
	if (OwnerController)
	{
...
void UGAITask_MoveTo::OnDestroy(bool bInOwnerFinished)
{
...
void UGAITask_MoveTo::OnRequestFinished(FAIRequestID RequestID, const FPathFollowingResult& Result)
{
	if (RequestID == MoveRequestID)
	{
		...
		else
		{
			...
			if (bUseContinuousTracking && MoveRequest.IsMoveToActorRequest() && Result.IsSuccess())
			{
				...
				GetWorld()->GetTimerManager().SetTimerForNextTick(this, &UGAITask_MoveTo::PerformMove);
			}
			else
			{
...
void UGAITask_MoveTo::OnPathEvent(FNavigationPath* InPath, ENavPathEvent::Type Event)
{
...
void UGAITask_MoveTo::ConditionalUpdatePath()
{
	...
	{
		UE_VLOG(GetGameplayTasksComponent(), LogGameplayTasks, Log, TEXT("%s> can't path right now, waiting..."), *GetName());
		OwnerController->GetWorldTimerManager().SetTimer(PathRetryTimerHandle, this, &UGAITask_MoveTo::ConditionalUpdatePath, 0.2f, false);
	}
	else
	{
		PathRetryTimerHandle.Invalidate();
