...
UPixel2DTDPawnSensingComponent::UPixel2DTDPawnSensingComponent(const FObjectInitializer& ObjectInitializer)
...
void UPixel2DTDPawnSensingComponent::SetPeripheralVisionAngle(const float NewPeripheralVisionAngle)
{
...
void UPixel2DTDPawnSensingComponent::InitializeComponent()
{
...
void UPixel2DTDPawnSensingComponent::SetSensingUpdatesEnabled(const bool bEnabled)
{
...
void UPixel2DTDPawnSensingComponent::SetTimer(const float TimeInterval)
{
	// Only necessary to update if we are the server
	AActor* const Owner = GetOwner();
	if (IsValid(Owner) && GEngine->GetNetMode(GetWorld()) < NM_Client)
	{
		Owner->GetWorldTimerManager().SetTimer(TimerHandle_OnTimer, this, &UPixel2DTDPawnSensingComponent::OnTimer, TimeInterval, false);
	}
}


void UPixel2DTDPawnSensingComponent::SetSensingInterval(const float NewSensingInterval)
{
...
void UPixel2DTDPawnSensingComponent::OnTimer()
{
...
AActor* UPixel2DTDPawnSensingComponent::GetSensorActor() const
{
...
AController* UPixel2DTDPawnSensingComponent::GetSensorController() const
{
	AActor* SensorActor = GetOwner();
	AController* Controller = Cast<AController>(SensorActor);
...
bool UPixel2DTDPawnSensingComponent::IsSensorActor(const AActor* Actor) const
{
	return (Actor == GetSensorActor());
}

bool UPixel2DTDPawnSensingComponent::HasLineOfSightTo(const AActor* Other) const
{
...
bool UPixel2DTDPawnSensingComponent::CanSenseAnything() const
{
	return (bSeePawns && OnSeePawn.IsBound());
}


void UPixel2DTDPawnSensingComponent::UpdateAISensing()
{
	AActor* const Owner = GetOwner();
	check(IsValid(Owner));
...
void UPixel2DTDPawnSensingComponent::SensePawn(APawn& Pawn)
{
	// Visibility checks
...
void UPixel2DTDPawnSensingComponent::BroadcastOnSeePawn(APawn& Pawn)
{
	OnSeePawn.Broadcast(&Pawn);
}


bool UPixel2DTDPawnSensingComponent::CouldSeePawn(const APawn *Other, bool bMaySkipChecks) const
{
	if (!Other)
	{
		return false;
...
FVector UPixel2DTDPawnSensingComponent::GetSensorLocation() const
{
...
FRotator UPixel2DTDPawnSensingComponent::GetSensorRotation() const
{
...
#if WITH_EDITOR
void UPixel2DTDPawnSensingComponent::PostEditChangeProperty(FPropertyChangedEvent & PropertyChangedEvent)
{
...
bool UPixel2DTDPawnSensingComponent::ShouldCheckVisibilityOf(APawn *Pawn) const
{
...
void UPixel2DTDPawnSensingComponent::SetStartAngle(float NewAngle)
{
	StartAngle = NewAngle;
	StartAngleCosine = FMath::Cos(FMath::DegreesToRadians(StartAngle));
	StartAngleSine = FMath::Sin(FMath::DegreesToRadians(StartAngle));
...
void UPixel2DTDPawnSensingComponent::SetEndAngle(float NewAngle)
{
	EndAngle = NewAngle;
	EndAngleCosine = FMath::Cos(FMath::DegreesToRadians(EndAngle));
	EndAngleSine = FMath::Sin(FMath::DegreesToRadians(EndAngle));
