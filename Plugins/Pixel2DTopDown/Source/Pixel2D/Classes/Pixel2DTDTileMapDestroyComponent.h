...
#include "Pixel2DTDTileMapDestroyComponent.generated.h"

struct PIXEL2DTD_API FPixel2DTDTileMapDestroyedCell
{
	int32 Layer;
	int32 PosX;
	int32 PosY;

	FPixel2DTDTileMapDestroyedCell(int32 Layer, int32 PosX, int32 PosY)
		: Layer(Layer), PosX(PosX), PosY(PosY)
	{}

	bool operator==(const FPixel2DTDTileMapDestroyedCell& Other) const
	{
		return (Layer == Other.Layer) && (PosX == Other.PosX) && (PosY == Other.PosY);
	}

	friend inline int32 GetTypeHash(const FPixel2DTDTileMapDestroyedCell& Handle)
	{
...
UCLASS(ClassGroup = Pixel2DTopDown, meta = (BlueprintSpawnableComponent))
class PIXEL2DTD_API UPixel2DTDTileMapDestroyComponent : public UActorComponent
{
...
public:
	...
	void AddHiddenTile(int32 Layer, int32 PosX, int32 PosY, FPaperTileInfo &TileInfo)
	{
		...
		FPixel2DTDTileMapDestroyedCell Cell(Layer, PosX, PosY);
	...
	void RestoreHiddenTile(UPaperTileMap * TileMap)
	{
		...
		{
			FPixel2DTDTileMapDestroyedCell TilePos = Elem.Key;
...
private:
	TMap<FPixel2DTDTileMapDestroyedCell, FPaperTileInfo> HiddenCells;
