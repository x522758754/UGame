...
FPixel2DAnimBlueprintCompilerContext::FPixel2DAnimBlueprintCompilerContext(UPixel2DTDAnimBlueprint* SourceSketch, FCompilerResultsLog& InMessageLog, const FKismetCompilerOptions& InCompileOptions)
...
{
	SpriteAnimSchema = NewObject<UPixel2DTDAnimGraphSchema>();
	return SpriteAnimSchema;
}

void FPixel2DAnimBlueprintCompilerContext::CreateEvaluationHandlerStruct(UPixel2DTDAnimGraphNode_Base* VisualAnimNode, FEvaluationHandlerRecord& Record)
{
...
void FPixel2DAnimBlueprintCompilerContext::CreateEvaluationHandlerInstance(UPixel2DTDAnimGraphNode_Base* VisualAnimNode, FEvaluationHandlerRecord& Record)
{
	// Shouldn't create a handler if there is nothing to work with
	check(Record.ServicedProperties.Num() > 0);
...
void FPixel2DAnimBlueprintCompilerContext::ProcessAnimationNode(UPixel2DTDAnimGraphNode_Base* VisualAnimNode)
{
	...
	const UPixel2DTDAnimGraphSchema* AnimGraphDefaultSchema = GetDefault<UPixel2DTDAnimGraphSchema>();

	FEdGraphPinType NodeVariableType;
	NodeVariableType.PinCategory = UPixel2DTDAnimGraphSchema::PC_Struct;
	...
	UPixel2DTDAnimGraphNode_Base* TrueSourceObject = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimGraphNode_Base>(VisualAnimNode);
	...
	if (UPixel2DTDAnimGraphNode_StateMachineBase* StateMachineInstance = Cast<UPixel2DTDAnimGraphNode_StateMachineBase>(VisualAnimNode))
	{
	...
	for (auto SourcePinIt = VisualAnimNode->Pins.CreateIterator(); SourcePinIt; ++SourcePinIt)
	{
		...
		if ((SourcePin->Direction == EGPD_Input) && (AnimGraphDefaultSchema->IsPosePin(SourcePin->PinType)))
		{
			...
			FPixel2DTDPoseLinkMappingRecord LinkRecord = VisualAnimNode->GetLinkIDLocation(NodeType, SourcePin);
	...
	for (TFieldIterator<FProperty> NodePropIt(NodeType); NodePropIt; ++NodePropIt)
	{
		...
		{
			if (StructProp->Struct == FPaper2DTDExposedValueHandler::StaticStruct())
			{
...
int32 FPixel2DAnimBlueprintCompilerContext::GetAllocationIndexOfNode(UPixel2DTDAnimGraphNode_Base* VisualAnimNode)
{
...
void FPixel2DAnimBlueprintCompilerContext::PruneIsolatedAnimationNodes(const TArray<UPixel2DTDAnimGraphNode_Base*>& RootSet, TArray<UPixel2DTDAnimGraphNode_Base*>& GraphNodes)
{
	struct FNodeVisitorDownPoseWires
	{
		TSet<UEdGraphNode*> VisitedNodes;
		const UPixel2DTDAnimGraphSchema* Schema;

		FNodeVisitorDownPoseWires()
		{
			Schema = GetDefault<UPixel2DTDAnimGraphSchema>();
		}
	...
	{
		UPixel2DTDAnimGraphNode_Base* RootNode = *RootIt;
		Visitor.TraverseNodes(RootNode);
	}

	for (int32 NodeIndex = 0; NodeIndex < GraphNodes.Num(); ++NodeIndex)
	{
		UPixel2DTDAnimGraphNode_Base* Node = GraphNodes[NodeIndex];
...
void FPixel2DAnimBlueprintCompilerContext::ProcessAnimationNodesGivenRoot(TArray<UPixel2DTDAnimGraphNode_Base*>& AnimNodeList, const TArray<UPixel2DTDAnimGraphNode_Base*>& RootSet)
{
	...
	{
		UPixel2DTDAnimGraphNode_Base* VisualAnimNode = *SourceNodeIt;
		ProcessAnimationNode(VisualAnimNode);
	}
}
...
void FPixel2DAnimBlueprintCompilerContext::ProcessAllAnimationNodes()
{
	...
	TArray<UPixel2DTDAnimGraphNode_Base*> AnimNodeList;
	ConsolidatedEventGraph->GetNodesOfClass<UPixel2DTDAnimGraphNode_Base>(/*out*/ AnimNodeList);
	...
	TArray<UK2Node_FlipbookGetterTD*> RootGraphAnimGetters;
	ConsolidatedEventGraph->GetNodesOfClass<UK2Node_FlipbookGetterTD>(RootGraphAnimGetters);

	// Find the root node
	UPixel2DTDAnimGraphNode_Root* PrePhysicsRoot = NULL;
	TArray<UPixel2DTDAnimGraphNode_Base*> RootSet;
	...
	for (auto SourceNodeIt = AnimNodeList.CreateIterator(); SourceNodeIt; ++SourceNodeIt)
	{
		UPixel2DTDAnimGraphNode_Base* SourceNode = *SourceNodeIt;
		UPixel2DTDAnimGraphNode_Base* TrueNode = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimGraphNode_Base>(SourceNode);
		TrueNode->BlueprintUsage = EBlueprintUsage::NoProperties;

		if (UPixel2DTDAnimGraphNode_Root* PossibleRoot = Cast<UPixel2DTDAnimGraphNode_Root>(SourceNode))
		{
			if (UPixel2DTDAnimGraphNode_Root* Root = ExactCast<UPixel2DTDAnimGraphNode_Root>(PossibleRoot))
			{
	...
	//    }
		...
		// Wire root getters
		for (UK2Node_FlipbookGetterTD* RootGraphGetter : RootGraphAnimGetters)
		{
			AutoWireFlipbookGetter(RootGraphGetter, nullptr);
		}

		// Wire nested getters
		for (UK2Node_FlipbookGetterTD* Getter : FoundGetterNodes)
		{
...
int32 FPixel2DAnimBlueprintCompilerContext::ExpandGraphAndProcessNodes(UEdGraph* SourceGraph, UPixel2DTDAnimGraphNode_Base* SourceRootNode, UPixel2DTDAnimStateTransitionNode* TransitionNode, TArray<UEdGraphNode*>* ClonedNodes)
{
	...
	UPixel2DTDAnimGraphNode_Base* TargetRootNode = NULL;
	TArray<UPixel2DTDAnimGraphNode_Base*> AnimNodeList;
	TArray<UK2Node_TransitionRuleGetter*> Getters;
	TArray<UK2Node_FlipbookGetterTD*> AnimGetterNodes;
	...
	for (auto NodeIt = ClonedGraph->Nodes.CreateIterator(); NodeIt; ++NodeIt)
	{
		...
		else if (UK2Node_FlipbookGetterTD* NewGetterNode = Cast<UK2Node_FlipbookGetterTD>(Node))
		{
			AnimGetterNodes.Add(NewGetterNode);
		}
		else if (UPixel2DTDAnimGraphNode_Base* TestNode = Cast<UPixel2DTDAnimGraphNode_Base>(Node))
		{
	...
	for (UK2Node_FlipbookGetterTD* GetterNode : AnimGetterNodes)
	{
...
void FPixel2DAnimBlueprintCompilerContext::ProcessStateMachine(UPixel2DTDAnimGraphNode_StateMachineBase* StateMachineInstance)
{
	struct FMachineCreator
	{
	public:
		int32 MachineIndex;
		TMap<UPixel2DTDAnimStateNodeBase*, int32> StateIndexTable;
		TMap<UPixel2DTDAnimStateTransitionNode*, int32> TransitionIndexTable;
		UPixel2DTDAnimBlueprintGeneratedClass* AnimBlueprintClass;
		UPixel2DTDAnimGraphNode_StateMachineBase* StateMachineInstance;
		FCompilerResultsLog& MessageLog;
	public:
		FMachineCreator(FCompilerResultsLog& InMessageLog, UPixel2DTDAnimGraphNode_StateMachineBase* InStateMachineInstance, int32 InMachineIndex, UPixel2DTDAnimBlueprintGeneratedClass* InNewClass)
		...
		FPixel2DTDStateMachineDebugData& GetMachineSpecificDebugData()
		{
			UPixel2DTDAnimStateMachineGraph* SourceGraph = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimStateMachineGraph>(StateMachineInstance->EditorStateMachineGraph);
			return AnimBlueprintClass->GetAnimBlueprintDebugData().StateMachineDebugData.FindOrAdd(SourceGraph);
		}

		int32 FindOrAddState(UPixel2DTDAnimStateNodeBase* StateNode)
		{
			...
			else
			{
				...
				UPixel2DTDAnimStateNodeBase* SourceNode = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimStateNodeBase>(StateNode);
		...
		int32 FindOrAddTransition(UPixel2DTDAnimStateTransitionNode* TransitionNode)
		{
	...
	TMap<UPixel2DTDAnimGraphNode_TransitionResult*, int32> AlreadyMergedTransitionList;
	...
	for (auto StateNodeIt = StateMachineInstance->EditorStateMachineGraph->Nodes.CreateIterator(); StateNodeIt; ++StateNodeIt)
	{
		...
		if (UPixel2DTDAnimStateEntryNode* EntryNode = Cast<UPixel2DTDAnimStateEntryNode>(Node))
		{
			...
			else if (UPixel2DTDAnimStateNodeBase* StartState = Cast<UPixel2DTDAnimStateNodeBase>(EntryNode->GetOutputNode()))
			{
		...
		else if (UPixel2DTDAnimStateTransitionNode* TransitionNode = Cast<UPixel2DTDAnimStateTransitionNode>(Node))
		{
			...
			UPixel2DTDAnimStateNodeBase* PreviousState = TransitionNode->GetPreviousState();
			UPixel2DTDAnimStateNodeBase* NextState = TransitionNode->GetNextState();
		...
		else if (UPixel2DTDAnimStateNode* StateNode = Cast<UPixel2DTDAnimStateNode>(Node))
		{
			...
			if (StateNode->BoundGraph != NULL)
			{
				...
				if (UPixel2DTDAnimGraphNode_StateResult* AnimGraphResultNode = CastChecked<UPixel2DTDAnimStateGraph>(StateNode->BoundGraph)->GetResultNode())
				{
		...
		else if (UPixel2DTDAnimStateConduitNode* ConduitNode = Cast<UPixel2DTDAnimStateConduitNode>(Node))
		{
			...
			{
				if (UPixel2DTDAnimGraphNode_TransitionResult* EntryRuleResultNode = CastChecked<UPixel2DTDAnimTransitionGraph>(ConduitNode->BoundGraph)->GetResultNode())
				{
	...
	{
		UPixel2DTDAnimStateNodeBase* StateNode = StateNodeIt.Key();
		...
		for (UPixel2DTDAnimGraphNode_AssetPlayerBase* Node : AssetPlayerNodes)
		{
		...
		TArray<class UPixel2DTDAnimStateTransitionNode*> TransitionList;
		...
		{
			UPixel2DTDAnimStateTransitionNode* TransitionNode = *TransitionIt;
			const int32 TransitionIndex = Oven.FindOrAddTransition(TransitionNode);
			...
			if (UPixel2DTDAnimGraphNode_TransitionResult* TransitionResultNode = CastChecked<UPixel2DTDAnimTransitionGraph>(TransitionNode->BoundGraph)->GetResultNode())
			{
...
void FPixel2DAnimBlueprintCompilerContext::CopyTermDefaultsToDefaultObject(UObject* DefaultObject)
{
	...
	TMap<UPixel2DTDAnimGraphNode_Base*, int32> LinkIndexMap;
	TMap<UPixel2DTDAnimGraphNode_Base*, uint8*> NodeBaseAddresses;
	...
	for (TFieldIterator<FProperty> It(DefaultObject->GetClass(), EFieldIteratorFlags::ExcludeSuper); It; ++It)
	{
		...
		if (UPixel2DTDAnimGraphNode_Base* VisualAnimNode = AllocatedNodePropertiesToNodes.FindRef(TargetProperty))
		{
	...
	{
		FPixel2DTDPoseLinkMappingRecord& Record = *PoseLinkIt;

		UPixel2DTDAnimGraphNode_Base* LinkingNode = Record.GetLinkingNode();
		UPixel2DTDAnimGraphNode_Base* LinkedNode = Record.GetLinkedNode();
	...
	UPixel2DTDAnimBlueprintGeneratedClass* AnimBlueprintGeneratedClass = CastChecked<UPixel2DTDAnimBlueprintGeneratedClass>(NewClass);

	UPixel2DTDAnimInstance* DefaultAnimInstance = CastChecked<UPixel2DTDAnimInstance>(AnimBlueprintGeneratedClass->GetDefaultObject());
//
	...
	for (const FEvaluationHandlerRecord& EvaluationHandler : ValidEvaluationHandlerList)
	{
		if (EvaluationHandler.ServicedProperties.Num() > 0)
		{
			...
			UPixel2DTDAnimGraphNode_Base* Node = CastChecked<UPixel2DTDAnimGraphNode_Base>(Handler.CopyRecords[0].DestPin->GetOwningNode());
			UPixel2DTDAnimGraphNode_Base* TrueNode = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimGraphNode_Base>(Node);

			FPaper2DTDExposedValueHandler* HandlerPtr = EvaluationHandler.EvaluationHandlerProperty->ContainerPtrToValuePtr<FPaper2DTDExposedValueHandler>(EvaluationHandler.NodeVariableProperty->ContainerPtrToValuePtr<void>(DefaultAnimInstance));
...
void FPixel2DAnimBlueprintCompilerContext::MergeUbergraphPagesIn(UEdGraph* Ubergraph)
{
	...
	else
	{
		...
		for (int32 i = 0; i < Blueprint->FunctionGraphs.Num(); ++i)
		{
			...
			if (SourceGraph->Schema->IsChildOf(UPixel2DTDAnimGraphSchema::StaticClass()))
			{
...
void FPixel2DAnimBlueprintCompilerContext::ProcessOneFunctionGraph(UEdGraph* SourceGraph, bool bInternalFunction)
{
	...
	else if (SourceGraph->Schema->IsChildOf(UPixel2DTDAnimStateMachineSchema::StaticClass()))
	{
...
{
	if (TargetUClass && !((UObject*)TargetUClass)->IsA(UPixel2DTDAnimBlueprintGeneratedClass::StaticClass()))
	{
...
{
	NewSpriteAnimBlueprintClass = FindObject<UPixel2DTDAnimBlueprintGeneratedClass>(Blueprint->GetOutermost(), *NewClassName);

	if (NewSpriteAnimBlueprintClass == NULL)
	{
		NewSpriteAnimBlueprintClass = NewObject<UPixel2DTDAnimBlueprintGeneratedClass>(Blueprint->GetOutermost(), FName(*NewClassName), RF_Public | RF_Transactional);
	}
	else
	{
...
{
	NewSpriteAnimBlueprintClass = CastChecked<UPixel2DTDAnimBlueprintGeneratedClass>(ClassToUse);
}
...
void FPixel2DAnimBlueprintCompilerContext::PostCompile()
{
	...
	for (FStructProperty* Property : TFieldRange<FStructProperty>(AnimBlueprintGeneratedClass, EFieldIteratorFlags::IncludeSuper))
	{
		...
		{
			FPixel2DTDAnimNode_Base* AnimNode = Property->ContainerPtrToValuePtr<FPixel2DTDAnimNode_Base>(DefaultAnimInstance);
...
void FPixel2DAnimBlueprintCompilerContext::PostCompileDiagnostics()
{
	...
	if (!bIsDerivedAnimBlueprint)
	{
		...
		{
			if (UPixel2DTDAnimGraphNode_Base* Node = NodeIt.Key())
			{
...
void FPixel2DAnimBlueprintCompilerContext::AutoWireFlipbookGetter(class UK2Node_FlipbookGetterTD* Getter, UPixel2DTDAnimStateTransitionNode* InTransitionNode)
{
	UEdGraphPin* ReferencedNodeTimePin = nullptr;
	int32 ReferencedNodeIndex = INDEX_NONE;
	int32 SubNodeIndex = INDEX_NONE;

	UPixel2DTDAnimGraphNode_Base* ProcessedNodeCheck = NULL;

	if (UPixel2DTDAnimGraphNode_Base* SourceNode = Getter->SourceNode)
	{
		UPixel2DTDAnimGraphNode_Base* ActualSourceNode = MessageLog.FindSourceObjectTypeChecked<UPixel2DTDAnimGraphNode_Base>(SourceNode);

		if (UPixel2DTDAnimGraphNode_Base* ProcessedSourceNode = SourceNodeToProcessedNodeMap.FindRef(ActualSourceNode))
		{
	...
	if (Getter->SourceStateNode)
	{
		...
		if (UPixel2DTDAnimStateNode* SourceStateNode = Cast<UPixel2DTDAnimStateNode>(SourceObject))
		{
			if (FPixel2DTDStateMachineDebugData* DebugData = NewSpriteAnimBlueprintClass->GetAnimBlueprintDebugData().StateMachineDebugData.Find(SourceStateNode->GetGraph()))
			{
...
{
	FPaper2DTDExposedValueHandler* HandlerPtr = EvaluationHandlerProperty->ContainerPtrToValuePtr<FPaper2DTDExposedValueHandler>(NodeVariableProperty->ContainerPtrToValuePtr<void>(TargetObject));
	...
	if (IsFastPath())
	{
		for (const TPair<FName, FAnimNodeSinglePropertyHandler>& ServicedPropPair : ServicedProperties)
		{
			...
			for (const FPropertyCopyRecord& PropertyCopyRecord : PropertyHandler.CopyRecords)
			{
				...
				FPixel2DTDExposedValueCopyRecord CopyRecord;
