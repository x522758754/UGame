...
struct FPixel2DTDPoseLinkMappingRecord;
...
class UPixel2DTDAnimGraphNode_StateResult;
...
public:
	FPixel2DAnimBlueprintCompilerContext(UPixel2DTDAnimBlueprint* SourceSketch, FCompilerResultsLog& InMessageLog, const FKismetCompilerOptions& InCompileOptions);
...
protected:
	...
	protected:
		...
		public:
			int32 MachineIndex;
			int32 StateIndex;
			UPixel2DTDAnimGraphNode_StateResult* ResultNode;
	
		public:
			FStateRootNodeIndexFixup(int32 InMachineIndex, int32 InStateIndex, UPixel2DTDAnimGraphNode_StateResult* InResultNode)
	...
	protected:
		UPixel2DTDAnimBlueprintGeneratedClass* NewSpriteAnimBlueprintClass;
		UPixel2DTDAnimBlueprint* AnimBlueprint;
	
		UPixel2DTDAnimGraphSchema* SpriteAnimSchema;
	
		// Map of allocated v3 nodes that are members of the class
		TMap<class UPixel2DTDAnimGraphNode_Base*, FProperty*> AllocatedAnimNodes;
		TMap<FProperty*, class UPixel2DTDAnimGraphNode_Base*> AllocatedNodePropertiesToNodes;
		...
		TMap<class UPixel2DTDAnimGraphNode_Base*, UPixel2DTDAnimGraphNode_Base*> SourceNodeToProcessedNodeMap;
		...
		TMap<class UPixel2DTDAnimGraphNode_Base*, int32> AllocatedAnimNodeIndices;
	
		// Map from pose link LinkID address
		//@TODO: Bad structure for a list of these
		TArray<FPixel2DTDPoseLinkMappingRecord> ValidPoseLinkList;
		...
		TArray<class UK2Node_FlipbookGetterTD*> FoundGetterNodes;
	...
	private:
		...
		void CreateEvaluationHandlerStruct(UPixel2DTDAnimGraphNode_Base* VisualAnimNode, FEvaluationHandlerRecord& Record);
		void CreateEvaluationHandlerInstance(UPixel2DTDAnimGraphNode_Base* VisualAnimNode, FEvaluationHandlerRecord& Record);
	
		// Prunes any nodes that aren't reachable via a pose link
		void PruneIsolatedAnimationNodes(const TArray<UPixel2DTDAnimGraphNode_Base*>& RootSet, TArray<UPixel2DTDAnimGraphNode_Base*>& GraphNodes);
	
		// Compiles one animation node
		void ProcessAnimationNode(UPixel2DTDAnimGraphNode_Base* VisualAnimNode);
	
		// Compiles one state machine
		void ProcessStateMachine(UPixel2DTDAnimGraphNode_StateMachineBase* StateMachineInstance);
	
		// Compiles an entire animation graph
		void ProcessAllAnimationNodes();
	
		//
		void ProcessAnimationNodesGivenRoot(TArray<UPixel2DTDAnimGraphNode_Base*>& AnimNodeList, const TArray<UPixel2DTDAnimGraphNode_Base*>& RootSet);

		// Automatically fill in parameters for the specified Getter node
		void AutoWireFlipbookGetter(class UK2Node_FlipbookGetterTD* Getter, UPixel2DTDAnimStateTransitionNode* InTransitionNode);
		...
		int32 ExpandGraphAndProcessNodes(UEdGraph* SourceGraph, UPixel2DTDAnimGraphNode_Base* SourceRootNode, UPixel2DTDAnimStateTransitionNode* TransitionNode = NULL, TArray<UEdGraphNode*>* ClonedNodes = NULL);

		// Returns the allocation index of the specified node, processing it if it was pending
		int32 GetAllocationIndexOfNode(UPixel2DTDAnimGraphNode_Base* VisualAnimNode);
};
