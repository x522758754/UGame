...
void FPixel2DAnimBlueprintEditor::InitSpriteAnimBlueprintEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UPixel2DTDAnimBlueprint* InSpriteAnimBlueprint)
{
	if (!Toolbar.IsValid())
	{
...
UBlueprint* FPixel2DAnimBlueprintEditor::GetBlueprintObj() const
{
	...
	{
		if (EditingObjs[i]->IsA<UPixel2DTDAnimBlueprint>()) { return (UBlueprint*)EditingObjs[i]; }
	}
	return nullptr;
}


UPixel2DTDAnimBlueprint* FPixel2DAnimBlueprintEditor::GetSpriteAnimBlueprint() const
{
	return Cast<UPixel2DTDAnimBlueprint>(GetBlueprintObj());
}
...
void FPixel2DAnimBlueprintEditor::OnOpenRelatedAsset()
{
	...
	if (SelectedNodes.Num() > 0)
	{
		...
		{
			if (UPixel2DTDAnimGraphNode_AssetSprite* Node = Cast<UPixel2DTDAnimGraphNode_AssetSprite>(*NodeIter))
			{
