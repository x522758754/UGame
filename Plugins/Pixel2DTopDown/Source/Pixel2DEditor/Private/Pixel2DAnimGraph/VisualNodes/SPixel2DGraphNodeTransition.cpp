...
void SPixel2DGraphNodeTransition::Construct(const FArguments& InArgs, UPixel2DTDAnimStateTransitionNode* InNode)
{
...
{
	UPixel2DTDAnimStateTransitionNode* TransNode = CastChecked<UPixel2DTDAnimStateTransitionNode>(GraphNode);
	...
	if ((PrevState != NULL) && (NextState != NULL))
	{
		...
		if ((pPrevNodeWidget != NULL) && (pNextNodeWidget != NULL))
		{
			...
			TArray<UPixel2DTDAnimStateTransitionNode*> Transitions;
			PrevState->GetTransitionList(Transitions);

			Transitions = Transitions.FilterByPredicate([NextState](const UPixel2DTDAnimStateTransitionNode* InTransition) -> bool
			{
...
TSharedRef<SWidget> SPixel2DGraphNodeTransition::GenerateRichTooltip()
{
	...
	if (UPixel2DTDAnimTransitionGraph* TransGraph = Cast<UPixel2DTDAnimTransitionGraph>(TransNode->BoundGraph))
	{
		if (UPixel2DTDAnimGraphNode_TransitionResult* ResultNode = TransGraph->GetResultNode())
		{
...
FText SPixel2DGraphNodeTransition::GetPreviewCornerText(bool bReverse) const
{
	...
	UPixel2DTDAnimStateNodeBase* PrevState = (bReverse ? TransNode->GetNextState() : TransNode->GetPreviousState());
	UPixel2DTDAnimStateNodeBase* NextState = (bReverse ? TransNode->GetPreviousState() : TransNode->GetNextState());
	...
	if (PrevState != NULL)
	{
		if (NextState != NULL)
		{
			TArray<UPixel2DTDAnimStateTransitionNode*> TransitionFromSource;
...
FLinearColor SPixel2DGraphNodeTransition::StaticGetTransitionColor(UPixel2DTDAnimStateTransitionNode* TransNode, bool bIsHovered)
{
