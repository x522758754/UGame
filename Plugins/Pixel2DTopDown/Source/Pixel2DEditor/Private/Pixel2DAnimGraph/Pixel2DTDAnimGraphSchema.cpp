...
#define LOCTEXT_NAMESPACE "Pixel2DTDAnimGraphSchema"

/////////////////////////////////////////////////////
// UPixel2DTDAnimGraphSchema

UPixel2DTDAnimGraphSchema::UPixel2DTDAnimGraphSchema(const FObjectInitializer& ObjectInitializer)
...
FLinearColor UPixel2DTDAnimGraphSchema::GetPinTypeColor(const FEdGraphPinType& PinType) const
{
	const bool bAdditive = PinType.PinSubCategory == TEXT("Additive");
	if (UPixel2DTDAnimGraphSchema::IsLocalSpacePosePin(PinType))
	{
		if (bAdditive)
		{
...
EGraphType UPixel2DTDAnimGraphSchema::GetGraphType(const UEdGraph* TestEdGraph) const
{
	return GT_Animation;
}

void UPixel2DTDAnimGraphSchema::CreateDefaultNodesForGraph(UEdGraph& Graph) const
{
	//// Create the result node
	FGraphNodeCreator<UPixel2DTDAnimGraphNode_Root> NodeCreator(Graph);
	UPixel2DTDAnimGraphNode_Root* ResultSinkNode = NodeCreator.CreateNode();
	NodeCreator.Finalize();
	SetNodeMetaData(ResultSinkNode, FNodeMetadata::DefaultGraphNode);
}

void UPixel2DTDAnimGraphSchema::HandleGraphBeingDeleted(UEdGraph& GraphBeingRemoved) const
{
	if (UBlueprint* Blueprint = FBlueprintEditorUtils::FindBlueprintForGraph(&GraphBeingRemoved))
	{
		...
		FBlueprintEditorUtils::GetAllNodesOfClassEx<UPixel2DTDAnimStateNode>(Blueprint, StateNodes);
...
bool UPixel2DTDAnimGraphSchema::IsPosePin(const FEdGraphPinType& PinType)
{
	return IsLocalSpacePosePin(PinType);
}

bool UPixel2DTDAnimGraphSchema::IsLocalSpacePosePin(const FEdGraphPinType& PinType)
{
	const UPixel2DTDAnimGraphSchema* Schema = GetDefault<UPixel2DTDAnimGraphSchema>();

	UScriptStruct* PoseLinkStruct = FPixel2DTDPoseLink::StaticStruct();
	return (PinType.PinCategory == Schema->PC_Struct) && (PinType.PinSubCategoryObject == PoseLinkStruct);
}

bool UPixel2DTDAnimGraphSchema::TryCreateConnection(UEdGraphPin* A, UEdGraphPin* B) const
{
...
const FPinConnectionResponse UPixel2DTDAnimGraphSchema::DetermineConnectionResponseOfCompatibleTypedPins(const UEdGraphPin* PinA, const UEdGraphPin* PinB, const UEdGraphPin* InputPin, const UEdGraphPin* OutputPin) const
{
...
bool UPixel2DTDAnimGraphSchema::ArePinsCompatible(const UEdGraphPin* PinA, const UEdGraphPin* PinB, const UClass* CallingContext, bool bIgnoreArray) const
{
...
bool UPixel2DTDAnimGraphSchema::DoesSupportAnimNotifyActions() const
{
	// Don't offer notify items in anim graph
	return false;
}

bool UPixel2DTDAnimGraphSchema::SearchForAutocastFunction(const UEdGraphPin* OutputPin, const UEdGraphPin* InputPin, FName& TargetFunction, /*out*/ UClass*& FunctionOwner) const
{
	return Super::SearchForAutocastFunction(OutputPin, InputPin, TargetFunction, FunctionOwner);
}

bool UPixel2DTDAnimGraphSchema::CreateAutomaticConversionNodeAndConnections(UEdGraphPin* PinA, UEdGraphPin* PinB) const
{
...
void UPixel2DTDAnimGraphSchema::SpawnNodeFromAsset(UPaperFlipbook* Asset, const FVector2D& GraphPosition, UEdGraph* Graph, UEdGraphPin* PinIfAvailable)
{
	check(Graph);
	check(Graph->GetSchema()->IsA(UPixel2DTDAnimGraphSchema::StaticClass()));
	check(Asset);

	UPixel2DTDAnimBlueprint* AnimBlueprint = Cast<UPixel2DTDAnimBlueprint>(FBlueprintEditorUtils::FindBlueprintForGraph(Graph));

	const bool bTypeMatch = (PinIfAvailable == NULL) || UPixel2DTDAnimGraphSchema::IsLocalSpacePosePin(PinIfAvailable->PinType);
	...
	if (bTypeMatch && bDirectionMatch)
	{
		...
		if (NewNodeClass)
		{
			check(NewNodeClass->IsChildOf(UPixel2DTDAnimGraphNode_AssetPlayerBase::StaticClass()));

			UPixel2DTDAnimGraphNode_AssetPlayerBase* NewNode = NewObject<UPixel2DTDAnimGraphNode_AssetPlayerBase>(GetTransientPackage(), NewNodeClass);
...
void UPixel2DTDAnimGraphSchema::UpdateNodeWithAsset(UK2Node* K2Node, UPaperFlipbook* Asset)
{
	// REVISIT
}

void UPixel2DTDAnimGraphSchema::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraph* Graph) const
{
...
void UPixel2DTDAnimGraphSchema::DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraphNode* Node) const
{
	UPaperFlipbook* Asset = FAssetData::GetFirstAsset<UPaperFlipbook>(Assets);
...
void UPixel2DTDAnimGraphSchema::DroppedAssetsOnPin(const TArray<FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraphPin* Pin) const
{
	UPaperFlipbook* Asset = FAssetData::GetFirstAsset<UPaperFlipbook>(Assets);
...
void UPixel2DTDAnimGraphSchema::GetContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	Super::GetContextMenuActions(Menu,Context);
}

FText UPixel2DTDAnimGraphSchema::GetPinDisplayName(const UEdGraphPin* Pin) const
{
	check(Pin != NULL);
