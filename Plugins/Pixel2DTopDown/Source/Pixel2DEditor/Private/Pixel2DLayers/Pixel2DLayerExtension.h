...
private:
	...
	TArray<TSharedPtr<FPixel2DTDLayer>> Layers;
	TSharedPtr<FPixel2DTableRow> RowDelegate;
	TSharedPtr<SListView<TSharedPtr<FPixel2DTDLayer>>> LayerList;
	TSharedPtr<FPixel2DTableDelegate> TableDelegate;

	void AddLayer();
	void DeleteLayer(TSharedPtr<FPixel2DTDLayer> DeletedLayer);
	void ChangeDepth(TSharedPtr<FPixel2DTDLayer> Layer, bool Increment);

	TSharedRef<ITableRow> OnGenerateWidgetForList(TSharedPtr<FPixel2DTDLayer> InItem, const TSharedRef<STableViewBase>& OwnerTable);

};



class FPixel2DTableRow : public SMultiColumnTableRow<TSharedPtr<FPixel2DTDLayer>>
{

public:

   SLATE_BEGIN_ARGS(FPixel2DTableRow) : _RowLayerData() {}
		SLATE_ARGUMENT(TSharedPtr<FPixel2DTDLayer>, RowLayerData)
...
public:
	...
	void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView)
	{
		...
		SMultiColumnTableRow<TSharedPtr<FPixel2DTDLayer>>::Construct(SMultiColumnTableRow<TSharedPtr<FPixel2DTDLayer>>::FArguments().Padding(1), InOwnerTableView);
	};
...
private:
	TSharedPtr<FPixel2DTDLayer>           RowLayerData;
	...
	ECheckBoxState GetIsVisible(TSharedPtr<FPixel2DTDLayer> data)
	{
		if (data->isVisible)
	...
	ECheckBoxState GetIsLocked(TSharedPtr<FPixel2DTDLayer> data)
	{
		if (data->isLocked)
			return ECheckBoxState::Checked;
