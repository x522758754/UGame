...
TSharedRef<SDockTab> FPixel2DLayerExtension::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs)
{
	...
	Layers = UPixel2DTDLayerManager::GetLayers();   
	
	SAssignNew(LayerList, SListView<TSharedPtr<FPixel2DTDLayer>>)
	.ItemHeight(27)
...
{
	UPixel2DTDLayerManager::AddLayer();
	Layers = UPixel2DTDLayerManager::GetLayers();
	LayerList->RebuildList();
}

void FPixel2DLayerExtension::DeleteLayer(TSharedPtr<FPixel2DTDLayer> DeletedLayer)
{
	UPixel2DTDLayerManager::DeleteLayer(DeletedLayer);
	Layers = UPixel2DTDLayerManager::GetLayers();
	LayerList->RebuildList();
}

void FPixel2DLayerExtension::ChangeDepth(TSharedPtr<FPixel2DTDLayer> Layer, bool Increment)
{
	UPixel2DTDLayerManager::ChangeDepth(Layer, Increment);
	Layers = UPixel2DTDLayerManager::GetLayers();
	LayerList->RebuildList();
}

TSharedRef<ITableRow> FPixel2DLayerExtension::OnGenerateWidgetForList(TSharedPtr<FPixel2DTDLayer> InItem, const TSharedRef<STableViewBase>& OwnerTable)
{ 
